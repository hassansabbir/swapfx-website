...
import { Search, SlidersHorizontal, ChevronDown, Shield, CheckCircle2, Star, ArrowRight, ArrowLeft, ArrowRight as ArrowRightIcon } from "lucide-react";
...
const MarketMainPage = () => {
  ...
  const renderFlag = (c: string) => {
    ...
    if (c === "UK") {
      return (
        <span className="flex items-center gap-2">
          <svg viewBox="0 0 60 30" className="w-5 h-3.5 rounded-sm shrink-0 shadow-sm border border-slate-100/50">
      ...
      return (
        <span className="flex items-center gap-2">
          <svg viewBox="0 0 30 20" className="w-5 h-3.5 rounded-sm shrink-0 shadow-sm border border-slate-100/50">
            ...
            <circle cx="15" cy="10" r="4" fill="none" stroke="#ffcc00" strokeWidth="1" strokeDasharray="1 1.5" />
          </svg>
          <span>Europe</span>
        </span>
      );
      ...
      <GlassContainer className="p-4 md:p-10 space-y-8">
        <div className="space-y-6 max-w-[750px] mx-auto">
          ...
          <div className="flex gap-3 w-full">
            ...
            <button className="w-12 h-12 shrink-0 rounded-xl bg-[#09A6A4] text-white flex items-center justify-center hover:bg-[#078d8b] transition-all shadow-md shadow-[#09A6A4]/20 hover:scale-[1.02]">
                ...
                <div className="flex justify-between items-baseline px-2 border-t border-slate-50 pt-5 text-[0.88rem]">
                  <div className="space-y-0.5">
                    <p className="text-slate-600 font-medium leading-none">
                      <span className="font-bold text-slate-800">Rate:</span> {offer.rate}
                    </p>
                  ...
                  <p className="text-slate-600 font-medium leading-none">
                    <span className="font-bold text-slate-800">Required:</span> {offer.requiredWithin}
                  </p>
                </div>

                {/* Centered View Details Action Button */}
                <div className="flex justify-center pt-2 w-full">
                  <Link href="/swap/confirmation?from=market" className="w-full">
