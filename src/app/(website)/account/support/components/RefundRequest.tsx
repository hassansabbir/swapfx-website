...
interface RefundRequestProps {
  ...
  setRefundFeeType: (val: string) => void;
  refundSwapId: string;
  setRefundSwapId: (val: string) => void;
  refundReason: string;
  setRefundReason: (val: string) => void;
  refundDetail: string;
  setRefundDetail: (val: string) => void;
  onBack: () => void;
  onSubmit: () => void;
}
    ...
    <div className="max-w-[700px] mx-auto w-full space-y-6 pt-4 relative animate-in fade-in duration-300">
      ...
      <div className="space-y-5 pt-4">
        ...
        <div className="space-y-2">
          ...
          <button className="flex items-center gap-2 px-4 py-2 border border-slate-200 bg-white rounded-lg text-slate-700 text-[0.88rem] hover:bg-slate-50 transition-colors shadow-xs focus:outline-none">
            <Upload size={16} />
